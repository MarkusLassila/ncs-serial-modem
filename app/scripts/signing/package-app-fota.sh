#!/usr/bin/env bash
#
# package-app-fota.sh - build dfu_application.zip from the signed app.
#
# Wraps the signed application .bin into the same FOTA package the NCS build
# would produce (dfu_application.zip), using NCS's own generate_zip.py so the
# manifest matches what nRF Cloud / fota_download expect. App image only.
#
# Required (produced by sign-assemble.sh):
...
"signed app binary not found: ${INPUT_BIN}
       Run sign-assemble.sh first, or pass --input <app_signed.bin>."
[ -f "${MANIFEST}" ] || err \
"manifest not found: ${MANIFEST}
       Expected from sign-assemble.sh, or pass --manifest <file>."
