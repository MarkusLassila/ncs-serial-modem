...
static void poll_work_fn(struct k_work *work)
{
	...
	for (int i = 0; i < SM_MAX_SOCKET_COUNT; i++) {
		...
		uint8_t revents = atomic_clear(&sock->async_poll.revents) & UINT8_MAX;
...
static int do_socket_open(struct sm_socket *sock)
{
	...
	update_poll_events(sock, sock->async_poll.xapoll_events_requested, true);
