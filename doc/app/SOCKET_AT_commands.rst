...
* The ``<handle>`` value sets the socket handle to poll.
  ...
  If handle is omitted, the operation will apply to all open sockets and to any new sockets that are created.
...
The set command allows you to configure the socket mode of all sockets or a specific socket.
...
::
   ...
   AT#XSOCKETMODE=[<handle>],<auto_reception_flags>[,<hex_format>]

* The ``<handle>`` parameter is an integer that identifies the socket handle.
  If omitted, the command applies to all opened sockets.
...
::
   ...
   // Create a new socket to show that the socket mode settings for all sockets are retained.
   AT#XSOCKET=1,1,0
   #XSOCKET: 1,1,6
   OK
   ...
   // Disable automatic data reception in AT-command mode in binary format for all sockets.
   AT#XSOCKETMODE=,0
   OK
...
The read command allows you to check the socket mode settings of sockets.
...
::
   ...
   #SOCKETMODE: <handle>,<auto_reception_flags>,<hex_format>
...
::
   ...
   #XSOCKETMODE: <handle>,(0,1,2,3),(0,1)
